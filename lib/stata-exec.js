...
module.exports = {
  ...
  runAll() {
    ...
    var msg = '';
    if (whichApp == "XQuartz") {
      msg = 'Running entire do file not supported for XQuartz';
      console.error(msg);
      this.conditionalWarning(msg);
      return;
    }
  ...
  run(advancePosition=false, batch=false) {
    ...
    this.sendCode(selection.selection, whichApp, batch);
  ...
  sendCode(code, whichApp, batch=false) {
    ...
    if (String(code).length > 8192 & (whichApp != "XQuartz" & process.platform != 'linux')) {
      var msg = 'Code selection must be fewer than 8192 characters';
      console.error(msg);
      this.conditionalWarning(msg);
      return;
    } else {
        ...
        if (this.con) {
          ...
          delay(2000)
            .then(() => {
              ...
              cwd = editor.getPath();
              folder_path = cwd.substring(0, cwd.lastIndexOf('\\') + 1);
  ...
  runParagraph() {
    ...
    if (paragraphRange) {
      ...
      var msg = 'No paragraph at cursor.';
      console.error(msg);
      return this.conditionalWarning(msg);
    }
  },
  ...
  setWorkingDirectory() {
    ...
    var msg = '';
    if (whichApp == "XQuartz") {
      msg = 'Set Working Directory not supported for XQuartz';
      console.error(msg);
      this.conditionalWarning(msg);
      return;
    }
    ...
    if (!cwd) {
      msg = 'No current working directory (save the file first).';
      console.error(msg);
      this.conditionalWarning(msg);
      return;
    }
