...
module.exports = {
  ...
  activate(state) {
    ...
    this.subscriptions.add('atom-text-editor:not([mini])', {
      'stata-exec:run': () => this.run(),
      'stata-exec:run-previous-command': () => this.runPreviousCommand(),
      'stata-exec:run-all': () => this.runAll(),
      'stata-exec:run-paragraph': () => this.runParagraph(),
      'stata-exec:run-program': () => this.runFunction(),
  ...
  stata_linux(selection) {
    ...
    fs.writeFileSync(path.join(process.env.HOME, '.stata-exec_code'), selection);
