...
module.exports = {
  ...
  runAll() {
    ...
    return sendCode.sendCode(doFileCommand.addSlashes(), whichApp);
  },
  ...
  run(advancePosition=false, batch=false) {
    ...
    sendCode.sendCode(selection.selection, whichApp, batch);
  ...
  runPreviousCommand() {
    const whichApp = atom.config.get('stata-exec.whichApp');
    return sendCode.sendCode(sendCode.previousCommand, whichApp);
  },
  ...
  runFunction() {
    ...
    if (range != null) {
      ...
      return sendCode.sendCode(code, whichApp);
    } else {
  ...
  runParagraph() {
    ...
    if (paragraphRange) {
      ...
      code = code.addSlashes();
      sendCode.sendCode(code, whichApp);
  ...
  setWorkingDirectory() {
    ...
    return sendCode.sendCode(cwd.addSlashes(), whichApp);
  }
};
