...
module.exports = {
  ...
  runAll() {
    ...
    if (whichApp == "XQuartz") {
      var msg = 'Running entire do file not supported for XQuartz';
      console.error(msg);
    ...
    if (!documentTitle) {
      var msg = 'Error: File not yet saved.';
      console.error(msg);
      this.conditionalWarning(msg);
      return;
    }
  ...
  setWorkingDirectory() {
    const whichApp = atom.config.get('stata-exec.whichApp');
    if (whichApp == "XQuartz") {
      var msg = 'Set Working Directory not supported for XQuartz';
      console.error(msg);
      this.conditionalWarning(msg);
      return;
    }
    ...
    if (!cwd) {
      var msg = 'No current working directory (save the file first).';
      console.error(msg);
      this.conditionalWarning(msg);
      return;
    }
