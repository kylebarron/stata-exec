...
module.exports = {
  ...
  removeComments(code) {
    ...
    // First group captures quoted double strings (should include `"..."')
    // Second group matches /* ... */, /// ..., // ...
    var code = 'abc "// hello //" "//" // comment //';
...
var re = new RegExp(/((["'])(?:\\[\s\S]|.)*?\2|\/(?![*\/])(?:\\.|\[(?:\\.|.)\]|.)*?\/)|\/\/.*?$|\/\*[\s\S]*?\*\//gm);
var code = 'abc "// hello //" "//" // comment //';
code.replace(/((["'])(?:\\[\s\S]|.)*?\2|\/(?![*\/])(?:\\.|\[(?:\\.|.)\]|.)*?\/)|\/\/.*?$|\/\*[\s\S]*?\*\//gm, $1)
...
code.
  ...
  function replacer(match, ) {
    ...
    var re_newstring = new RegExp(/([^\d]*)(\d*)([^\w]*)/g);
    var newString = 'abc12345#$*%abc'.replace(re_newstring, replacer);
...
anotherString = someString.replace(/cat/g, 'dog');
  ...
  _getSurroundingCharacters() {
    ...
    const currentPosition = editor.getCursorBufferPosition();
    const leftPosition = Point(currentPosition.row, currentPosition.column - 1);
    const rightPosition = Point(currentPosition.row, currentPosition.column + 1);
